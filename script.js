...
function getFilteredProducts() {
  return getAudienceProducts().filter(product => {
    if (state.activeFilter === 'new' && !product.isNew) return false;
    if (state.activeFilter === 'last' && product.stockLevel !== 'low') return false;
    if (!['all', 'new', 'last'].includes(state.activeFilter) && product.category !== state.activeFilter) return false;
...
function selectAudience(audience, { scroll = true } = {}) {
  ...
  document.getElementById('catalogEyebrow').textContent = `Catalogue ${label}`;
...
function renderTicker() {
  const items = ['DROP ACTIF', 'STOCK MIS À JOUR', 'PHOTOS RÉELLES', 'RÉSERVATION DIRECTE', 'QUANTITÉS LIMITÉES', 'PACKS VISION'];
...
function renderFilters() {
  ...
  if (!bar) return;
  const categories = [...new Set(getAudienceProducts().map(product => product.category))];
  const filters = [...SPECIAL_FILTERS, ...categories.map(category => ({ key: category, label: category }))];
...
function fallbackImageHTML(id) {
  return `<div class="card-img-fallback"><span class="fallback-logo">VISION</span><span class="fallback-id">PHOTO À AJOUTER · ${escHtml(id)}</span></div>`;
}
...
function packImageHTML(pack) {
  ...
  return `<div class="pack-img-fallback"><span>OUTFIT · ${escHtml(pack.id)}</span></div>`;
}
...
function renderPacks() {
  ...
  grid.querySelectorAll('img[data-pack-fallback]').forEach(image => {
    image.addEventListener('error', () => {
      ...
      fallback.innerHTML = `<span>OUTFIT · ${escHtml(image.dataset.packFallback)}</span>`;
...
function modalImageHTML(item, type) {
  ...
  if (!image) return `<div class="modal-img-fallback"><span class="fb-logo">VISION</span><small>PHOTO À AJOUTER · ${escHtml(item.id)}</small></div>`;
...
function renderModalContent(item, type) {
  ...
  modalImage?.addEventListener('error', () => {
    modalImage.parentElement.innerHTML = `<div class="modal-img-fallback"><span class="fb-logo">VISION</span></div>`;
  }, { once: true });
