...
function renderPackOutfitSection(gender = state.audience) {
  ...
  const savingHeadline = Number.isFinite(pricing?.savings)
    ? `Tu gardes ${savingText}`
    : 'Prix pack activé';
  const discountText = Number.isFinite(pricing?.savings)
    ? `-${pricing.savingPercent}% sur ton pack`
    : `Pack ${mode} articles · -${pricing?.discountPercent || 0}% dès prix confirmé`;

  document.getElementById('packEyebrow').textContent = 'Stock limité · Prix Pack';
  document.getElementById('packTitle').textContent = 'Pack Outfit VISION';
  document.getElementById('packSubtitle').textContent = '2 ou 3 pièces. Prix réduit.';
  ...
  root.innerHTML = `
    <div class="vision-pack-smart is-mode-${mode} reveal" data-pack-builder="${escHtml(gender)}">
      <div class="vision-pack-smart-controls" aria-label="Choix rapide Pack Outfit">
        ...
        <div class="vision-pack-toggle vision-pack-mode-toggle" role="group" aria-label="Nombre d’articles du pack">
          <button type="button" class="${mode === 2 ? 'active' : ''}" data-pack-mode="2" data-pack-gender="${escHtml(gender)}" aria-pressed="${mode === 2}">
            <span>Pack 2 articles</span><small>Économie rapide</small>
          </button>
      ...
      <div class="vision-pack-benefit">
        <span>${mode === 3 ? 'Max économie' : 'Économie rapide'}</span>
        <strong>Plus rentable que l’achat séparé</strong>
      </div>
      ...
      <div class="vision-pack-economy" aria-label="Résumé économie Pack Outfit">
        ...
        <div class="vision-pack-economy-grid">
          ...
          <div><span>Tu gardes</span><strong>${escHtml(savingText)}</strong></div>
        </div>
        ...
        <div class="vision-pack-actions">
          ...
          <button class="vision-pack-snap" type="button" data-pack-snap="${escHtml(gender)}" ${packReady ? '' : 'disabled'}>Réserver ce pack sur Snap</button>
        </div>
        <small class="vision-pack-trust">Stock limité · Réservation directe · Prix pack selon disponibilité</small>
      </div>
    </div>`;
