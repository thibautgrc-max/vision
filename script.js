...
function renderPackOutfitSection(gender = state.audience) {
  ...
  const pricing = updatePackPricing(gender);
  const label = gender === 'femme' ? 'Femme' : 'Homme';
  const characterVideo = gender === 'femme' ? 'assets/videos/vision-femme.mp4' : 'assets/videos/vision-homme.mp4';
  ...
  document.getElementById('packSubtitle').textContent = 'Compose ton outfit avec une pièce, une paire et un bonus optionnel. Le pack se prépare directement selon le stock disponible.';
  ...
  root.innerHTML = `
    <div class="vision-pack-builder reveal" data-pack-builder="${escHtml(gender)}">
      <div class="vision-pack-selectors" aria-label="Sélection Pack Outfit ${escHtml(label)}">
        ...
        ${renderPackSelectorCard(gender, 'footwear', { label: 'Chaussures / Claquettes', badge: 'Paire assortie' }, lists.footwear, active.footwearIndex)}
        ${renderPackSelectorCard(gender, 'bonus', { label: 'Bonus optionnel', badge: 'Parfum · Accessoire · Électronique' }, lists.bonus, active.bonusIndex)}
      </div>
...
function renderModalContent(item, type) {
  ...
  const quickTrust = !isPack
    ? `<div class="quick-trust">
        <div><span aria-hidden="true">◆</span><p>Pièce disponible dans le stock privé VISION.</p></div>
        <div><span aria-hidden="true">◆</span><p>Réservation rapide sur Snapchat.</p></div>
        <div><span aria-hidden="true">◆</span><p>Disponibilité confirmée avant validation.</p></div>
      </div>`
    : '';
